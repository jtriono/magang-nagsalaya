...
Public Class Nominal_bayar_piutang
    ...
    Private Sub btnbayar_Click(sender As Object, e As EventArgs) Handles btnbayar.Click
        'Try
        '    query = "update piutang set lunas = 1, tanggal_transaksi = '" + Form_Bayar_Piutang.dtptanggal.Value.ToString + "' where total_harga = '" + Form_Bayar_Piutang.simpanharga + "' and lunas = 0"
        '    connect.Open()
        ...
        ElseIf CInt(tbharga.Text) - CInt(tbinputbayar.Text) > 0 Then
            query = "update piutang set total_harga = '" + harga.ToString + "' where total_harga = '" + Form_Bayar_Piutang.simpanharga + "' and lunas = 0"
        End If
        connect.Open()
        command = New MySqlCommand(query, connect)
        command.ExecuteNonQuery()
        ...
        query = "insert into bayar_piutang Values('" + tbnota.Text + "','" + tbnama.Text + "', '" + tbinputbayar.Text + "', '" + Form_Bayar_Piutang.dtptanggal.Value.ToString + "',0)"
            connect.Open()
            command = New MySqlCommand(query, connect)
            command.ExecuteNonQuery()
            connect.Close()
